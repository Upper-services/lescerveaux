        ...
        <div className="item">
          ...
          {user && (
            <main className="relative min-h-screen after:bg-center after:bg-cover after:bg-no-repeat after:bg-fixed after:absolute after:inset-0 after:z-[-1] top-[72px]">
              ...
              <Fade bottom>
                <section className="grid grid-cols-2 items-center justify-center md:grid-cols-4 mt-10 gap-6 gap-y-7 px-8 max-w-[1200px] mx-auto">
                  {categoriesSSR.map((doc) => {
                    const { title, img, id } = doc
                    return <Category title={title} img={img} key={id} id={id} />
                  })}
                </section>
              </Fade>

              <div className="relative flex flex-col space-y-4 mt-10 mb-4 pl-5 md:pl-10 lg:pl-24">
                <HomeCollection
...
export async function getServerSideProps(context) {
  ...
  // Les Regles dor des 100 livres preferes de nadir
